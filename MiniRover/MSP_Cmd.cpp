...
void MSP_Cmd::SendResp( MSP_RESP_OBJ *pResp )
{
	...
	Serial1.write( MSP_CMD_START );
	Serial1.write( MSP_CMD_HEADER_M );

	if( pResp->ErrorCode > 0 )	Serial1.write('!');
	else						Serial1.write('>');

	Serial1.write( pResp->Length );		
	Serial1.write( pResp->Cmd );		CheckSum ^= pResp->Cmd;

	for( i=0; i<pResp->Length; i++ )
	{
		Serial1.write( pResp->Data[i] );
